/* ============================================================
   MARSON SENSUALITY — Logique du site
   En-tête, barre d'annonce, recherche, panier, favoris,
   compte client, commande, rendu des sections.
   ============================================================ */
...
const I = {
  ...
  wa:'<svg viewBox="0 0 24 24"><path d="M12 2a10 10 0 0 0-8.6 15L2 22l5.2-1.4A10 10 0 1 0 12 2zm5.1 13.9c-.2.6-1.2 1.2-1.7 1.2-.5.1-1 .2-3.4-.7-2.9-1.2-4.7-4.1-4.9-4.3-.1-.2-1.1-1.5-1.1-2.9s.7-2 1-2.3c.2-.3.5-.3.7-.3h.5c.2 0 .4 0 .6.5l.8 2c.1.2.1.4 0 .6l-.4.6-.5.5c-.1.2-.3.3-.1.6.2.3.8 1.3 1.7 2.1 1.2 1 2.1 1.4 2.4 1.5.3.1.5.1.7-.1l.9-1c.2-.3.4-.2.7-.1l2 .9c.3.2.5.3.6.4 0 .2 0 .8-.2 1.5z"/></svg>',
...
const users = ()=> LS.get("ms_users", {});
const session = ()=> LS.get("ms_session", null);

/* ---------------- En-tête + barre d'annonce ---------------- */
function mountChrome(){
  ...
  const announce = `<div class="announce" aria-live="polite">` +
    MS.announcements.map((m,i)=>`<span class="${i===0?"on":""}"><i class="star">✦</i>${m}<i class="star">✦</i></span>`).join("") +
    `</div>`;

  const header = `
  <header class="header">
    <div class="container header-in">
      <a class="brand" href="index.html">MARSON SENSUALITY<small>${MS.config.sousTitre}</small></a>
  ...
  // rotation douce des messages
  const msgs = $$(".announce span");
  if(msgs.length > 1){
    let i = 0;
    setInterval(()=>{ msgs[i].classList.remove("on"); i = (i+1)%msgs.length; msgs[i].classList.add("on"); }, 4600);
  }
  ...
  <footer class="footer">
    <div class="container">
      <div class="footer-grid">
        <div>
          <a class="brand" href="index.html">MARSON SENSUALITY<small>${c.sousTitre}</small></a>
          <p class="tagline">${c.slogan}</p>
          <div class="socials">
            <a href="${c.instagram}" target="_blank" rel="noopener" aria-label="Instagram">${I.ig}</a>
            <a href="${c.facebook}" target="_blank" rel="noopener" aria-label="Facebook">${I.fb}</a>
            <a href="${c.tiktok}" target="_blank" rel="noopener" aria-label="TikTok">${I.tt}</a>
          </div>
        </div>
        <div>
          <h4>Boutique</h4>
          <ul>
            ...
            <li><a href="boutique.html?cat=brumes">Brumes</a></li>
            <li><a href="boutique.html?cat=coffrets">Coffrets</a></li>
        ...
        <div>
          ...
          <ul>
            ...
            <li><a href="tel:${c.telephone.replace(/[^+\d]/g,"")}">${c.telephone}</a></li>
            <li><a href="contact.html">${c.horaires}</a></li>
            <li><a href="contact.html">${c.adresse}</a></li>
          </ul>
        </div>
      </div>
...
function mountSearch(){
    ...
    <div class="search-panel">
      <div class="container">
        <div class="search-field">
          ${I.search.replace("<svg","<svg style='width:22px;height:22px;stroke:var(--brown-2);fill:none;stroke-width:1.4'")}
        ...
        <div class="search-sugs">
          <span class="lab">Suggestions :</span>
  ...
  function draw(){
    ...
    if(!list.length){
      res.innerHTML = `<div class="search-empty">
        <p class="serif">Aucun résultat trouvé.</p>
        <p>Essayez avec un autre mot-clé.</p>
        <p style="margin-top:18px"><a class="link-u" href="boutique.html">Découvrir tous les produits</a></p>
      </div>`;
      return;
    }
  ...
  input.addEventListener("keydown", e=>{
    if(e.key === "Enter"){ window.location.href = "boutique.html?q=" + encodeURIComponent(input.value.trim()); }
  });
...
function cardHTML(p){
  ...
  <article class="p-card reveal">
    <a class="ph" href="produit.html?id=${p.id}" aria-label="${esc(p.nom)}">
      <img src="${p.img}" alt="${esc(p.nom)} — MARSON SENSUALITY" loading="lazy" decoding="async">
      ${badge}
    </a>
...
function mountDrawer(){
  ...
  dr.innerHTML = `
    ...
    <div class="drawer-foot">
      <div class="promo-row">
        <input id="promo-input" placeholder="Code promo" aria-label="Code promo">
        <button id="promo-apply">Appliquer</button>
      </div>
  ...
  $("#promo-apply", dr).addEventListener("click", ()=>{
    const code = ($("#promo-input", dr).value || "").trim().toUpperCase();
    const info = MS_DATA.promoInfo(code);
    if(info && info.valid){ LS.set("ms_promo", code); toast("Code " + code + " appliqué"); renderCart(); }
    else if(info && !info.valid){ toast("Ce code a expiré"); }
    else toast("Code promo invalide");
  });
...
function cartDetail(){
  ...
  const code = LS.get("ms_promo", null);
  const info = code ? MS_DATA.promoInfo(code) : null;
  const rate = info && info.valid ? info.rate : 0;
  const disc = Math.round(sub * rate);
  return {lines, sub, code: rate ? code : null, rate, disc, count: lines.reduce((s,l)=> s + l.qty, 0)};
}
...
function renderCart(){
  ...
  if(!d.lines.length){
    body.innerHTML = `<div class="cart-empty">
      <p class="serif">Votre panier est vide.</p>
    ...
    $("#go-checkout").style.pointerEvents = "none"; $("#go-checkout").style.opacity = ".4";
    return;
  }
  $("#go-checkout").style.pointerEvents = ""; $("#go-checkout").style.opacity = "";

  body.innerHTML = d.lines.map(l=>`
    <div class="cart-line">
      <img src="${l.p.img}" alt="${l.p.nom}">
      <div>
        <h4>${esc(l.p.nom)}</h4>
        <p class="u">${MS_UTILS.fmt(l.p.prix)} · ${l.p.format}</p>
        <div class="qty">
      ...
      <div class="right">
        <span class="sub">${MS_UTILS.fmt(l.p.prix * l.qty)}</span>
        <button class="rm" data-cart="rm" data-id="${l.p.id}">Retirer</button>
      </div>
    </div>`).join("");

  applied.innerHTML = d.code ? `<div class="promo-applied"><span>Code ${d.code} · −${Math.round(d.rate*100)} %</span><button class="rm" id="promo-rm">Retirer</button></div>` : "";
...
function addToCart(id, qty){
  ...
  if(p && p.stock === 0){ toast("Ce parfum est momentanément en rupture"); return; }
  if(p && typeof p.stock === "number" && p.stock > 0){
    const inCart = cart()[id] || 0;
    if(inCart + (qty||1) > p.stock){ toast("Stock disponible insuffisant (" + p.stock + ")"); return; }
  }
...
async function registerUser(data){
  const u = users();
  if(u[data.email]) return "Un compte existe déjà avec cet e-mail.";
  u[data.email] = { prenom:data.prenom, nom:data.nom, telephone:data.telephone, password: await hashPwd(data.password), adresses:[] };
  LS.set("ms_users", u); LS.set("ms_session", data.email);
  return null;
}
async function loginUser(email, pwd){
  const u = users()[email];
  if(!u) return "E-mail ou mot de passe incorrect.";
  const h = await hashPwd(pwd);
  if(u.password === h || (u.password === pwd)){ // compatibilité ancien stockage
    if(u.password === pwd){ u.password = h; const all = users(); all[email] = u; LS.set("ms_users", all); }
    LS.set("ms_session", email); return null;
  }
  return "E-mail ou mot de passe incorrect.";
}
function logout(){ localStorage.removeItem("ms_session"); toast("Vous êtes déconnecté(e)"); window.location.reload(); }
...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  if(nl) nl.addEventListener("submit", e=>{
    ...
    const list = LS.get("ms_newsletter", []); if(!list.includes(em)) list.push(em); LS.set("ms_newsletter", list);
...
/* ---------- Accueil ---------- */
function initHome(){
  // Slogan & phrases de marque (modifiables depuis le dashboard)
  const hs = $("#hero-slogan");
  if(hs){
    const s = MS.config.slogan, i = s.indexOf(",");
    hs.innerHTML = i > 0 ? s.slice(0, i+1) + "<br>" + s.slice(i+1).trim() : s;
  }
  ...
  $("#home-cats").innerHTML = MS.categories.map(c=>`
    <a class="cat-card reveal" href="boutique.html?cat=${c.id}">
      <div class="ph"><img src="${c.img}" alt="${c.nom}" loading="lazy"></div>
      <div class="txt"><h3 class="serif">${c.nom}</h3><span>${c.phrase}</span></div>
    </a>`).join("");
  ...
  if(fc){
    ...
    const fi = $("#feat-img");  if(fi) fi.src = fc.img;
    const fd = $("#feat-desc"); if(fd) fd.textContent = fc.phrase;
    const fl = $("#feat-link"); if(fl) fl.href = "collections.html?id=" + fc.id;
  }
  const sig = MS.products.filter(p => p.active !== false && fc && p.collections.includes(fc.id)).slice(0,3);
  $("#feat-mini").innerHTML = sig.map(p=>`
    <a href="produit.html?id=${p.id}"><img src="${p.img}" alt="${esc(p.nom)}" loading="lazy" decoding="async"><span>${esc(p.nom)}</span></a>`).join("");

  const ids = MS.featured.length ? MS.featured : ["vanille-sensuality","rose-eclat","ambre-nuit","voile-de-soie"];
  $("#home-prods").innerHTML = ids.map(id=> MS_UTILS.byId(id)).filter(p => p && p.active !== false).map(cardHTML).join("");
  ...
  $("#insta-grid").innerHTML = insta.map(src=>`<a href="${MS.config.instagram}" target="_blank" rel="noopener"><img src="${src}" alt="Instagram MARSON SENSUALITY" loading="lazy"></a>`).join("");

  $("#adv-grid").innerHTML = MS.advantages.map(a=>`
    <div class="adv reveal">${I[a.icone] || I.drop}<h3>${a.titre}</h3><p>${a.texte}</p></div>`).join("");

  // Données structurées (SEO) : organisation + recherche
  const ld = {
    "@context":"https://schema.org",
    "@type":"Organization",
    name:"MARSON SENSUALITY",
    slogan: MS.config.slogan,
    email: MS.config.email,
    telephone: MS.config.telephone,
    areaServed:"Côte d'Ivoire",
    sameAs:[MS.config.instagram, MS.config.facebook, MS.config.tiktok].filter(Boolean)
  };
...
function initBoutique(){
  ...
  const chips = [["tous","Tous les produits"]]
    .concat(MS.categories.map(c => [c.id, c.nom]))
    .concat([["nouveautes","Nouveautés"],["promotions","Promotions"]]);
  ...
  function draw(){
    ...
    if(!l.length){
      $("#shop-grid").innerHTML = `<div class="search-empty" style="grid-column:1/-1">
        <p class="serif">Aucun résultat trouvé.</p><p>Essayez avec un autre mot-clé.</p>
        <p style="margin-top:18px"><button class="link-u" id="reset-shop">Découvrir tous les produits</button></p></div>`;
      $("#reset-shop").addEventListener("click", ()=>{ cat = "tous"; q = ""; qInput.value = ""; $$("#shop-filters .chip").forEach(c=> c.classList.toggle("active", c.dataset.cat === "tous")); draw(); });
      return;
    }
...
function initCollections(){
  ...
  if(id){
    ...
    $("#coll-detail").innerHTML = `
      <div class="coll-hero">
        <img src="${c.img}" alt="${c.nom}">
        <div class="veil"></div>
        <div class="container txt">
          <span class="kicker" style="color:var(--gold)">Collection</span>
          <h1>${c.nom.replace("Collection ","Collection <em>") + "</em>"}</h1>
          <p>${c.phrase}</p>
        </div>
      </div>
      <div class="container">
    ...
    $("#coll-list").innerHTML = MS.collections.map(c=>`
      <a class="coll-card reveal" href="collections.html?id=${c.id}">
        ...
        <div class="veil"></div>
        <div class="txt">
          <h3>${c.nom.replace("Collection ","Collection <em>") + "</em>"}</h3>
          <p>${c.phrase}</p>
          <span class="link-u">Découvrir</span>
        </div>
      </a>`).join("");
  }
  if(window.__observeReveal) window.__observeReveal();
...
function initProduit(){
  ...
  if(!p){ wrap.innerHTML = `<div class="container search-empty"><p class="serif">Produit introuvable.</p><p style="margin-top:16px"><a class="link-u" href="boutique.html">Découvrir tous les produits</a></p></div>`; return; }
  ...
  <div class="container prod-layout" style="padding:70px 0">
    ...
    <div class="p-info">
      ...
      <div class="buy-row">
        ...
        <button class="btn" data-add="${p.id}" id="add-btn" style="flex:1;min-width:200px" ${out ? "disabled" : ""}>${out ? "Rupture de stock" : "Ajouter au panier"}</button>
      </div>
      <div class="p-actions">
      ...
      <div class="p-meta">
        <p><span>✦</span> Livraison à Abidjan et partout en Côte d'Ivoire — sous ${MS.config.livraison.delai}.</p>
  ...
  $("#add-btn").dataset.qty = qty;
  $("#add-btn").addEventListener("click", ()=>{ $("#add-btn").dataset.qty = qty; });

  const main = $("#g-main");
  const views = [
    {src: p.img,  t: ""},
    {src: p.img,  t: "scale(1.7)", p: "top center"},
    {src: p.img,  t: "scale(1.4)", p: "center"},
    {src: p.img2 || p.img, t: ""}
  ];
  $$(".thumbs button").forEach(b=> b.addEventListener("click", ()=>{
    ...
    main.src = v.src;
    main.style.transform = v.t || "";
    main.style.objectPosition = v.p || "center";
  ...
  const ld = {
    ...
    name: p.nom, image: [p.img, p.img2].filter(Boolean),
  ...
  const rel = MS.products.filter(x=> x.id !== p.id && x.collections.some(c=> p.collections.includes(c))).slice(0,3);
  const fill = rel.length ? rel : MS.products.filter(x=> x.id !== p.id).slice(0,3);
  $("#rel-grid").innerHTML = fill.map(cardHTML).join("");
  bindCards($("#rel-grid"));
  if(window.__observeReveal) window.__observeReveal();
}
...
function initCommande(){
  ...
  const d = cartDetail();
  if(!d.lines.length){
    wrap.innerHTML = `<div class="container cart-empty" style="padding:90px 0">
      <p class="serif">Votre panier est vide.</p>
  ...
  <div class="container order-grid" style="padding:70px 0">
    <form id="order-form" novalidate>
      ...
      <div class="field"><label for="o-zone">Zone de livraison</label>
        <select id="o-zone">
          ...
          <option value="interieur">Autre ville de Côte d'Ivoire</option>
        </select>
      </div>
      ...
      <div class="form-row">
        <div class="field"><label for="o-mode">Mode de livraison</label>
          <select id="o-mode">
            <option value="rdv">Point de rendez-vous</option>
            <option value="coursier">Coursier (livraison à l'adresse)</option>
          </select>
        </div>
        <div class="field"><label for="o-paiement">Paiement</label>
          <select id="o-paiement">
    ...
    <aside class="order-sum">
      ...
      ${d.lines.map(l=>`<div class="sum-line"><span class="l"><img src="${l.p.img}" alt=""><span>${l.p.nom}<br><small style="color:var(--muted)">× ${l.qty}</small></span></span><span>${MS_UTILS.fmt(l.p.prix*l.qty)}</span></div>`).join("")}
  ...
  let ship = 0;
  function totals(){
    const total = d.sub - d.disc + ship;
    $("#sum-totals").innerHTML = `
      <div class="tot-row"><span>Sous-total</span><span>${MS_UTILS.fmt(d.sub)}</span></div>
      ${d.disc ? `<div class="tot-row"><span>Remise (${d.code})</span><span>− ${MS_UTILS.fmt(d.disc)}</span></div>` : ""}
      <div class="tot-row"><span>Livraison</span><span>${ship ? MS_UTILS.fmt(ship) : "<i class='tbd'>à déterminer</i>"}</span></div>
      <div class="tot-row total"><span>Total</span><span>${MS_UTILS.fmt(total)}</span></div>`;
    return total;
  }
  totals();
  ...
  $("#order-form").addEventListener("submit", e=>{
    e.preventDefault();
    const req = ["o-prenom","o-nom","o-tel","o-mail","o-zone","o-adresse"];
    for(const id of req){
      const el = $("#"+id);
      if(!el.value.trim()){ el.focus(); el.style.borderColor = "#C4756A"; toast("Merci de compléter tous les champs"); return; }
      el.style.borderColor = "";
    }
    const n = LS.get("ms_order_seq", 1024) + 1; LS.set("ms_order_seq", n);
    const em = session();
    const order = {
      ref: "MS" + n,
      date: new Date().toLocaleDateString("fr-FR"),
      mois: new Date().toISOString().slice(0,7),
      client: $("#o-prenom").value.trim() + " " + $("#o-nom").value.trim(),
      email: $("#o-mail").value.trim(), tel: $("#o-tel").value.trim(),
      items: d.lines.map(l=> ({id:l.p.id, nom:l.p.nom, qty:l.qty, prix:l.p.prix})),
      sousTotal: d.sub, remise: d.disc, livraison: ship,
      total: d.sub - d.disc + ship,
      zone: $("#o-zone").value, mode: $("#o-mode").value, paiement: $("#o-paiement").value,
      adresse: $("#o-adresse").value.trim(), note: $("#o-note").value.trim(),
      compte: em ? "client" : "invité",
      statut: "En préparation"
    };
    const all = MS_DATA.orders(); all.push(order); MS_DATA.saveOrders(all);
    // décrémente le stock suivi
    d.lines.forEach(l=>{
      if(typeof l.p.stock === "number" && l.p.stock > 0){
        MS_DATA.saveProduct(Object.assign({}, l.p, {stock: Math.max(0, l.p.stock - l.qty)}));
      }
    });
    LS.set("ms_cart", {}); localStorage.removeItem("ms_promo"); renderCart();
    if(window.__msTrack) window.__msTrack("Purchase", {value: order.total, currency: "XOF"});
    wrap.innerHTML = `<div class="container order-done">
      <span class="kicker">Merci</span>
      <p class="serif">Votre commande est confirmée.</p>
      <p class="ref">Commande ${order.ref} · ${MS_UTILS.fmt(order.total)}</p>
      <p class="lead" style="max-width:460px;margin:0 auto 30px">Nous vous contacterons très vite pour confirmer la livraison (délai habituel : ${MS.config.livraison.delai}). Un récapitulatif vous a été envoyé par e-mail.</p>
      ${em ? "" : `<p style="margin-bottom:26px"><a class="link-u" href="compte.html">Créer un compte pour retrouver cette commande</a></p>`}
      <a class="btn" href="boutique.html">Continuer mes achats</a>
    </div>`;
...
function initCompte(){
  ...
  function drawAuth(){
    ...
    <div class="container-narrow" style="padding:70px 0">
      <div class="auth-tabs">
        <button class="on" data-tab="login">Connexion</button>
    ...
    tabs.forEach(t=> t.addEventListener("click", ()=>{
      ...
      t.dataset.tab === "login" ? loginForm() : regForm();
    }));
    function loginForm(){
      $("#auth-panel").innerHTML = `
        ...
        <form id="f-login">
          ...
          <p style="text-align:center;margin-top:16px"><button type="button" class="link-u" id="forgot">Mot de passe oublié ?</button></p>
        </form>`;
      $("#f-login").addEventListener("submit", async e=>{
        e.preventDefault();
        const err = await loginUser($("#l-mail").value.trim(), $("#l-pwd").value);
        if(err){ toast(err); return; }
        toast("Connexion réussie"); drawDash(session());
      });
      $("#forgot").addEventListener("click", ()=>{
        const m = $("#l-mail").value.trim();
        toast(m ? "Un e-mail de réinitialisation a été envoyé" : "Saisissez d'abord votre e-mail");
      });
    }
    function regForm(){
      $("#auth-panel").innerHTML = `
        ...
        <form id="f-reg">
          <div class="form-row">
            <div class="field"><label>Prénom</label><input id="r-prenom" required></div>
            <div class="field"><label>Nom</label><input id="r-nom" required></div>
          </div>
      ...
      $("#f-reg").addEventListener("submit", async e=>{
        ...
        const err = await registerUser({
          prenom:$("#r-prenom").value.trim(), nom:$("#r-nom").value.trim(),
          telephone:$("#r-tel").value.trim(), email:$("#r-mail").value.trim(),
          password:$("#r-pwd").value
        });
        if(err){ toast(err); return; }
        toast("Bienvenue dans votre espace"); drawDash(session());
      });
    }
    loginForm();
  }

  function drawDash(em){
    const u = users()[em];
    wrap.innerHTML = `
    ...
    nav.addEventListener("click", e=>{
      ...
      if(b.dataset.p === "out"){ logout(); return; }
    ...
    const panels = {
      infos(){
        $("#acc-panel").innerHTML = `
          <h2 class="serif">Bonjour, ${u.prenom}</h2>
          <div class="field"><label>Prénom</label><input id="i-prenom" value="${u.prenom}"></div>
          <div class="field"><label>Nom</label><input id="i-nom" value="${u.nom}"></div>
          <div class="field"><label>Téléphone</label><input id="i-tel" value="${u.telephone}"></div>
          <div class="field"><label>E-mail</label><input value="${em}" disabled style="opacity:.6"></div>
          <button class="btn" id="i-save">Enregistrer</button>`;
        $("#i-save").addEventListener("click", ()=>{
          u.prenom = $("#i-prenom").value; u.nom = $("#i-nom").value; u.telephone = $("#i-tel").value;
          const all = users(); all[em] = u; LS.set("ms_users", all); toast("Informations enregistrées");
        });
      },
      orders(){
        const ords = MS_DATA.orders().filter(o => o.email === em).slice().reverse();
        $("#acc-panel").innerHTML = `<h2 class="serif">Mes commandes</h2>` + (ords.length ? ords.map(o=>`
          <div class="order-card">
            <div class="top"><span>Commande ${o.ref}</span><span>${o.date}</span><span class="status">Statut : ${o.statut}</span></div>
            ${o.items.map(it=> `<p style="font-size:13px;color:var(--brown-2)">${it.nom} — ${MS_UTILS.fmt(it.prix)} × ${it.qty}</p>`).join("")}
      ...
      addr(){
        $("#acc-panel").innerHTML = `
          ...
          <div style="margin-top:24px">${(u.adresses||[]).map((a,i)=>`<p style="padding:12px 0;border-top:1px solid var(--line);font-size:13.5px;color:var(--brown-2)">${a} <button class="rm" data-del="${i}" style="float:right">Supprimer</button></p>`).join("") || `<p class="lead" style="margin-top:18px">Aucune adresse enregistrée.</p>`}</div>`;
        $("#a-add").addEventListener("click", ()=>{
          const v = $("#a-new").value.trim(); if(!v) return;
          u.adresses = u.adresses || []; u.adresses.push(v);
          const all = users(); all[em] = u; LS.set("ms_users", all); panels.addr(); toast("Adresse ajoutée");
        });
        $$("[data-del]").forEach(b=> b.addEventListener("click", ()=>{
          u.adresses.splice(parseInt(b.dataset.del,10),1);
          const all = users(); all[em] = u; LS.set("ms_users", all); panels.addr();
        }));
      },
      pwd(){
        $("#acc-panel").innerHTML = `
        ...
        $("#p-save").addEventListener("click", async ()=>{
          if($("#p-old").value && (await hashPwd($("#p-old").value)) !== u.password && u.password !== $("#p-old").value){ toast("Mot de passe actuel incorrect"); return; }
          if($("#p-new").value.length < 6){ toast("6 caractères minimum"); return; }
          u.password = await hashPwd($("#p-new").value); const all = users(); all[em] = u; LS.set("ms_users", all);
          toast("Mot de passe mis à jour"); panels.pwd();
        });
      }
    };
    panels.infos();
...
function initContact(){
  ...
  f.addEventListener("submit", e=>{
    e.preventDefault();
    const req = ["c-nom","c-mail","c-sujet","c-msg"];
    for(const id of req){
      const el = $("#"+id);
