/* ============================================================
   MARSON SENSUALITY — FRONTEND / dashboard administrateur
   Consomme l'API admin du backend (jeton Bearer, session 12 h).
   ============================================================ */
(function(){
"use strict";
...
let S = null;            // état complet venu du backend
let current = "home";
...
const validOrders = ()=> (S.orders || []).filter(o => o.statut !== "Annulée");
const byId = id => S.products.find(p => p.id === id);
const catName = id => { const c = S.categories.find(c=>c.id===id); return c ? c.nom.replace(/s\s*$/,"") : id; };
...
/* ---------- appel API admin avec gestion de session ---------- */
async function act(method, path, body){
  try{
    return await API.req(path, {method, body, admin: true});
  }catch(err){
    if(err.status === 401){ API.setAdminToken(null); showLogin(); toast("Session expirée — reconnectez-vous"); throw err; }
    toast(err.message); throw err;
  }
}
async function refresh(){ S = await act("GET", "/admin/state"); return S; }
...
async function show(panel, opts){
...
function renderHome(){
  ...
  $("#admin-main").innerHTML = `
    ...
    <div class="adm-stats">
      ...
      <div class="adm-stat"><span>Produits en ligne</span><b>${S.products.filter(p=>p.active!==false).length}</b><small>${S.products.filter(p=>p.active===false).length} désactivé(s)</small></div>
    </div>
    ${low.length ? `<div class="adm-card" style="border-color:var(--gold)"><h2>Stock <em>à surveiller</em></h2><p class="sub">${low.map(p=>esc(p.nom)+" ("+p.stock+")").join(" · ")}</p></div>` : ""}
    ${(S.messages||[]).length ? `<div class="adm-card"><h2>Messages <em>reçus</em> (formulaire de contact)</h2>
      ${(S.messages||[]).slice(-4).reverse().map(m=>`<p style="font-size:13px;color:var(--brown-2);padding:10px 0;border-bottom:1px solid var(--line)"><b style="font-weight:400">${esc(m.nom)}</b> · ${esc(m.sujet)} · ${esc(m.date)}<br>${esc(m.message)}</p>`).join("")}</div>` : ""}
    <div class="adm-quick">
...
function renderProducts(opts){
  ...
  function drawList(){
    ...
    const list = S.products.filter(p => !q || MS_UTILS.norm(p.nom + " " + p.type).includes(q));
    ...
    $$("[data-edit]").forEach(b => b.addEventListener("click", ()=> openForm(byId(b.dataset.edit))));
    $$("[data-toggle]").forEach(b => b.addEventListener("click", async ()=>{
      const p = byId(b.dataset.toggle);
      await act("POST", "/admin/products", {product: Object.assign({}, p, {active: p.active === false})});
      toast(p.active === false ? p.nom + " remis en ligne" : p.nom + " désactivé (masqué du site)");
      await show("products");
    }));
    $$("[data-del]").forEach(b => b.addEventListener("click", async ()=>{
      const p = byId(b.dataset.del);
      if(confirm(`Supprimer « ${p.nom} » de la boutique ?`)){
        await act("DELETE", "/admin/products/" + p.id);
        toast("Produit supprimé"); await show("products");
      }
    }));
  }
  ...
  function openForm(p){
    const isNew = !p;
    const d = p || { nom:"", type: S.categories[0] ? S.categories[0].id : "", audience:"femme", format:"Eau de parfum · 50 ml",
      prix:"", ancienPrix:null, nouveau:false, promo:false, img:"", img2:null, stock:null, active:true,
    ...
    <div class="adm-card" id="p-form-card">
      ...
      <div class="adm-form-grid">
        <div><div class="upload-zone" id="f-zone">${img ? `<img src="${img}" alt=""><p>Photo principale ✨ — touchez pour changer</p>` : `<p>✦ Photo principale</p><small>Touchez pour choisir depuis votre téléphone ou ordinateur</small>`}</div>
          <input type="file" id="f-file" accept="image/*" hidden></div>
        <div><div class="upload-zone" id="f-zone2">${img2 ? `<img src="${img2}" alt=""><p>Photo de mise en scène — touchez pour changer</p>` : `<p>Photo de mise en scène (optionnelle)</p><small>Flacon accompagné de tissu, fleurs, bijou…</small>`}</div>
          <input type="file" id="f-file2" accept="image/*" hidden></div>
        <div class="field"><label>✦ Nom du produit</label><input id="f-nom" value="${esc(d.nom)}" placeholder="Ex. : Vanille Sensuality"></div>
        <div class="field"><label>✦ Prix (FCFA)</label><input id="f-prix" inputmode="numeric" value="${d.prix === "" || d.prix == null ? "" : d.prix}" placeholder="Ex. : 22000"></div>
        <div class="field"><label>Catégorie</label><select id="f-cat">${S.categories.map(c=>`<option value="${c.id}" ${d.type===c.id?"selected":""}>${esc(c.nom)}</option>`).join("")}</select></div>
        <div class="field"><label>Pour qui</label><select id="f-aud">
          ...
          <option value="enfant" ${d.audience==="enfant"?"selected":""}>Enfant</option></select></div>
        <div class="field"><label>Format</label><input id="f-format" value="${esc(d.format)}"></div>
        ...
        <div class="field"><label>Visibilité</label>
          <label class="adm-check"><input type="checkbox" id="f-active" ${d.active === false ? "" : "checked"}> Produit en ligne (décocher = masqué)</label>
          <label class="adm-check"><input type="checkbox" id="f-nouveau" ${d.nouveau?"checked":""}> Étiquette « Nouveau »</label>
          <label class="adm-check"><input type="checkbox" id="f-promo" ${d.promo?"checked":""}> Étiquette « Promotion »</label></div>
        ...
        <div class="field"><label>Mots-clés de recherche</label><input id="f-key" value="${esc((d.keywords||[]).join(", "))}"></div>
        <div class="field full"><label>Collections</label><div class="adm-coll-checks">
          ${S.collections.map(c=>`<label class="adm-check"><input type="checkbox" class="f-coll" value="${c.id}" ${(d.collections||[]).includes(c.id)?"checked":""}> ${esc(c.nom)}</label>`).join("")}
        </div></div>
      </div>
      <div class="adm-actions"><button class="btn" id="f-save">Enregistrer le produit</button>
        <button class="btn btn-outline" id="f-cancel">Annuler</button></div>
    </div>`;
    const wire = (zone, file, set) => {
      $(zone).addEventListener("click", ()=> $(file).click());
      $(file).addEventListener("change", e=>{
        const f = e.target.files[0]; if(!f) return;
        readImage(f, v => { set(v); toast("Photo chargée ✨"); $(zone).innerHTML = `<img src="${v}" alt=""><p>Photo choisie — touchez pour changer</p>`; });
      });
    };
    wire("#f-zone", "#f-file", v => img = v);
    wire("#f-zone2", "#f-file2", v => img2 = v);
    $("#f-cancel").addEventListener("click", ()=>{ $("#p-form").innerHTML = ""; });
    $("#f-save").addEventListener("click", async ()=>{
      const nom = $("#f-nom").value.trim();
      const prix = parseInt(($("#f-prix").value || "").replace(/\s/g,""), 10);
      if(!nom){ toast("Donnez un nom au produit"); return; }
      if(!prix || prix <= 0){ toast("Indiquez un prix valide"); return; }
      ...
      let id = d.id;
      if(isNew){ id = slug(nom); let i = 2; while(byId(id)) id = slug(nom) + "-" + i++; }
      await act("POST", "/admin/products", {product: {
        id, nom, type: $("#f-cat").value, audience: $("#f-aud").value,
        format: $("#f-format").value.trim() || "—", prix,
        ancienPrix: ancien > 0 ? ancien : null,
        stock: stockRaw === "" ? null : Math.max(0, parseInt(stockRaw, 10) || 0),
        active: $("#f-active").checked, nouveau: $("#f-nouveau").checked, promo: $("#f-promo").checked,
...
function renderCatColl(){
  $("#admin-main").innerHTML = `
    <div class="adm-head">
      ...
      <p class="adm-guide">Les catégories rangent la boutique ; les collections racontent des univers. Les deux se créent et se modifient ici.</p>
    </div>
    <div class="adm-card">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;margin-bottom:18px">
        <h2 style="margin:0">Catégories</h2><button class="btn btn-sm" id="cat-new">+ Nouvelle catégorie</button></div>
      <div id="cat-list" style="overflow-x:auto"></div><div id="cat-form"></div>
    </div>
    <div class="adm-card">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;margin-bottom:18px">
        <h2 style="margin:0">Collections</h2><button class="btn btn-sm" id="coll-new">+ Nouvelle collection</button></div>
      <div id="coll-list" style="overflow-x:auto"></div><div id="coll-form"></div>
    </div>`;

  function drawCats(){
    $("#cat-list").innerHTML = `<table class="adm-table"><thead><tr><th>Catégorie</th><th>Phrase</th><th>Produits</th><th></th></tr></thead><tbody>
      ${S.categories.map(c=>`<tr><td><b style="font-weight:400;font-family:var(--serif);font-size:16px">${esc(c.nom)}</b></td>
        <td>${esc(c.phrase)}</td><td>${S.products.filter(p=>p.type===c.id).length}</td>
        <td><div class="adm-row-actions">
    ...
    $$("[data-cedit]").forEach(b=> b.addEventListener("click", ()=> catForm(S.categories.find(c=>c.id===b.dataset.cedit))));
    $$("[data-cdel]").forEach(b=> b.addEventListener("click", async ()=>{
      const c = S.categories.find(x=>x.id===b.dataset.cdel);
      if(S.products.some(p=>p.type===c.id)){ toast("Impossible : des produits utilisent cette catégorie"); return; }
      if(confirm(`Supprimer la catégorie « ${c.nom} » ?`)){
        try{ await act("DELETE", "/admin/categories/" + c.id); toast("Catégorie supprimée"); await show("catcoll"); }catch(e){}
      }
    }));
  }
  function catForm(c){
    ...
    $("#cat-form").innerHTML = `<div class="adm-card" style="margin-top:20px;background:var(--cream)">
      ...
      <div class="adm-form-grid">
        ...
        <div class="field"><label>Phrase courte</label><input id="cat-phrase" value="${esc(d.phrase)}"></div>
      ...
      <div class="adm-actions"><button class="btn btn-sm" id="cat-save">Enregistrer</button><button class="btn btn-outline btn-sm" id="cat-cancel">Annuler</button></div></div>`;
    ...
    $("#cat-save").addEventListener("click", async ()=>{
      ...
      while(isNew && S.categories.some(x=>x.id===id)) id = slug(nom) + "-" + i++;
      await act("POST", "/admin/categories", {category: {id, nom, phrase: $("#cat-phrase").value.trim(), img: img || d.img || ""}});
      toast("Catégorie enregistrée"); await show("catcoll");
    });
  }
  ...
  function drawColls(){
    $("#coll-list").innerHTML = `<table class="adm-table"><thead><tr><th>Collection</th><th>Univers</th><th>Produits</th><th></th></tr></thead><tbody>
      ${S.collections.map(c=>`<tr><td><b style="font-weight:400;font-family:var(--serif);font-size:16px">${esc(c.nom)}</b></td>
        <td>${esc(c.phrase)}</td><td>${S.products.filter(p=>(p.collections||[]).includes(c.id)).length}</td>
        <td><div class="adm-row-actions">
    ...
    $$("[data-coll-edit]").forEach(b=> b.addEventListener("click", ()=> collForm(S.collections.find(c=>c.id===b.dataset.collEdit))));
    $$("[data-coll-del]").forEach(b=> b.addEventListener("click", async ()=>{
      const c = S.collections.find(x=>x.id===b.dataset.collDel);
      if(confirm(`Supprimer la collection « ${c.nom} » ? Les produits resteront en boutique.`)){
        await act("DELETE", "/admin/collections/" + c.id);
        toast("Collection supprimée"); await show("catcoll");
      }
    }));
  }
  function collForm(c){
    ...
    $("#coll-form").innerHTML = `<div class="adm-card" style="margin-top:20px;background:var(--cream)">
      ...
      <div class="adm-form-grid">
        ...
        <div class="field full"><label>Phrase d'univers</label><input id="coll-phrase" value="${esc(d.phrase)}"></div>
        ...
        <div class="field full"><label>Produits associés</label><div class="adm-coll-checks">
          ${S.products.map(p=>`<label class="adm-check"><input type="checkbox" class="coll-prod" value="${p.id}" ${(p.collections||[]).includes(d.id||"")?"checked":""}> ${esc(p.nom)}</label>`).join("")}
        </div></div>
      </div>
      <div class="adm-actions"><button class="btn btn-sm" id="coll-save">Enregistrer</button><button class="btn btn-outline btn-sm" id="coll-cancel">Annuler</button></div></div>`;
    ...
    $("#coll-save").addEventListener("click", async ()=>{
      const nom = $("#coll-nom").value.trim(); if(!nom){ toast("Nom requis"); return; }
      let id = d.id || slug(nom); let i = 2;
      while(isNew && S.collections.some(x=>x.id===id)) id = slug(nom) + "-" + i++;
      await act("POST", "/admin/collections", {collection: {id, nom, phrase: $("#coll-phrase").value.trim(), img: img || d.img || ""}});
      await act("POST", "/admin/collections/products", {id, productIds: $$(".coll-prod").filter(x=>x.checked).map(x=>x.value)});
      toast("Collection enregistrée"); await show("catcoll");
    });
  }
...
function renderPromos(){
  ...
  const inPeriod = p => (!p.debut || today >= p.debut) && (!p.fin || today <= p.fin);
  $("#admin-main").innerHTML = `
    <div class="adm-head">
      <h1>Promotions & <em>codes</em></h1>
      <p class="adm-guide">Un code = une remise en pourcentage, avec dates de validité. Hors période, le code est refusé automatiquement.</p>
    </div>
    <div class="adm-card">
  ...
  function drawCodes(){
    const codes = S.promos || {};
    const keys = Object.keys(codes);
    $("#code-list").innerHTML = keys.length ? keys.map(k=>{
      const p = codes[k], ok = inPeriod(p);
      return `<div class="adm-list-input"><input value="${k} — ${Math.round(p.rate*100)} %${p.debut ? " · du " + p.debut : ""}${p.fin ? " au " + p.fin : ""}${ok ? "" : "  (hors validité)"}" disabled style="opacity:.8">
        <button data-cdel="${k}" title="Supprimer">✕</button></div>`;
    }).join("") : `<p class="adm-empty">Aucun code pour le moment.</p>`;
    $$("[data-cdel]").forEach(b=> b.addEventListener("click", async ()=>{
      await act("DELETE", "/admin/promos/" + b.dataset.cdel);
      toast("Code supprimé"); await show("promos");
    }));
  }
  $("#c-add").addEventListener("click", async ()=>{
    ...
    await act("POST", "/admin/promos", {code, rate: pct/100, debut: $("#c-debut").value || "", fin: $("#c-fin").value || ""});
  ...
  function drawPP(){
    ...
    const saveRow = async id => {
      ...
      await act("POST", "/admin/products", {product: Object.assign({}, p, {promo, ancienPrix: old > 0 ? old : null})});
...
function renderOrders(){
    ...
    <div class="adm-card" style="overflow-x:auto">
      ${all.length ? `<table class="adm-table"><thead><tr><th>Réf.</th><th>Date</th><th>Cliente / client</th><th>Total</th><th>Statut</th></tr></thead><tbody>
        ${all.map(o=>`<tr data-row="${o.ref}" style="cursor:pointer">
          ...
          <td>${fmt(o.total)}</td>
          <td><select data-stat="${o.ref}">${["En préparation","Expédiée","Livrée","Annulée"].map(s=>`<option ${o.statut===s?"selected":""}>${s}</option>`).join("")}</select></td></tr>
  ...
  $$("[data-stat]").forEach(s => s.addEventListener("change", async e=>{
    e.stopPropagation();
    await act("POST", "/admin/orders/status", {ref: s.dataset.stat, statut: s.value});
    toast(s.dataset.stat + " → " + s.value);
  }));
...
function renderAccounting(){
  ...
  $("#admin-main").innerHTML = `
    ...
    <div class="adm-card"><h2>Mois par <em>mois</em></h2>
      ${months.length ? months.map(k => `<div style="padding:10px 0;border-bottom:1px solid var(--line)">
        ...
        <div class="adm-bar"><i style="width:${Math.round(byMonth[k].ca / maxCa * 100)}%"></i></div></div>`).join("") : `<p class="adm-empty">Pas encore de ventes enregistrées.</p>`}
    </div>
    <div class="adm-card"><h2>Meilleurs <em>produits</em></h2>
      ${top.length ? top.map(k => `<div style="padding:10px 0;border-bottom:1px solid var(--line)">
        ...
        <div class="adm-bar"><i style="width:${Math.round(byProd[k].ca / maxP * 100)}%"></i></div></div>`).join("") : `<p class="adm-empty">Pas encore de ventes enregistrées.</p>`}
    </div>`;
  $("#csv-orders").addEventListener("click", ()=> dlCSV("marson-commandes.csv", [
    ...
    ...(S.orders||[]).map(o => [o.ref, o.date, o.client, o.email, o.tel, o.zone, o.mode, o.paiement, o.statut, o.sousTotal, o.remise, o.livraison, o.total, o.items.map(i=>i.nom+" x"+i.qty).join(" | ")])
  ]));
...
function renderSettings(){
  ...
  $("#admin-main").innerHTML = `
    <div class="adm-head">
      ...
      <p class="adm-guide">Textes du site, mise en avant, coordonnées, livraison, pixel Facebook, sauvegardes et sécurité — sans toucher au code.</p>
    </div>
    <div class="adm-card">
      <h2>Produits <em>mis en avant</em></h2>
      <p class="sub">« Les Incontournables » de l'accueil (4 recommandés) + collection mise en avant.</p>
      <div class="adm-coll-checks" id="feat-list">
        ${S.products.filter(p=>p.active!==false).map(p=>`<label class="adm-check"><input type="checkbox" class="feat-chk" value="${p.id}" ${(S.featured||[]).includes(p.id)?"checked":""}> ${esc(p.nom)}</label>`).join("")}
      </div>
      <div class="field" style="margin-top:16px"><label>Collection mise en avant sur l'accueil</label>
        <select id="feat-coll">${S.collections.map(x=>`<option value="${x.id}" ${S.featuredCollection===x.id?"selected":""}>${esc(x.nom)}</option>`).join("")}</select></div>
    ...
    <div class="adm-card">
      ...
      <div class="adm-form-grid">
        <div class="field full"><label>Slogan principal</label><input id="s-slogan" value="${esc(c.slogan)}"></div>
        <div class="field full"><label>Phrase « marque »</label><input id="s-marque" value="${esc(S.phrases.marque)}"></div>
        <div class="field full"><label>Phrase « collections »</label><input id="s-coll" value="${esc(S.phrases.collection)}"></div>
        <div class="field full"><label>Phrase « sensualité »</label><input id="s-sens" value="${esc(S.phrases.sensualite)}"></div>
      </div>
    ...
    <div class="adm-card">
      ...
      <p class="sub">Collez l'ID de votre pixel (Meta Events Manager). Vide = désactivé.</p>
    ...
    <div class="adm-card">
      ...
      <p class="sub">Exportez régulièrement une sauvegarde complète et conservez-la en lieu sûr.</p>
      ...
      <p style="font-size:13px;color:var(--brown-2)">Mesures actives côté serveur : mots de passe salés + SHA-256, session admin 12 h, verrouillage anti force-brute (5 essais / 5 min), prix et stocks recalculés à la commande, validation et échappement des contenus, contrôle d'accès par jeton. En production : hébergement HTTPS recommandé.</p>
  ...
  $("#feat-save").addEventListener("click", async ()=>{
    ...
    await act("POST", "/admin/featured", {ids, collection: $("#feat-coll").value});
    toast("Mise en avant enregistrée ✨");
  });
  function drawAnn(){
    $("#ann-list").innerHTML = S.announcements.map((a,i)=>`
      <div class="adm-list-input"><input data-ann="${i}" value="${esc(a)}"><button data-ann-del="${i}" title="Supprimer">✕</button></div>`).join("");
    $$("[data-ann-del]").forEach(b=> b.addEventListener("click", async ()=>{
      await act("POST", "/admin/announcements", {list: S.announcements.filter((_,i)=> i !== parseInt(b.dataset.annDel,10))});
      toast("Message retiré"); await show("settings");
    }));
  }
  $("#ann-add").addEventListener("click", async ()=>{
    await act("POST", "/admin/announcements", {list: S.announcements.concat(["Nouveau message"])});
    await show("settings");
  });
  $("#ann-save").addEventListener("click", async ()=>{
    await act("POST", "/admin/announcements", {list: $$("[data-ann]").map(i => i.value.trim()).filter(Boolean)});
    toast("Barre d'annonce mise à jour"); await show("settings");
  });
  drawAnn();
  $("#s-save").addEventListener("click", async ()=>{
    await act("POST", "/admin/config", {config: {slogan: $("#s-slogan").value.trim() || c.slogan}});
    await act("POST", "/admin/phrases", {phrases: {marque: $("#s-marque").value.trim(), collection: $("#s-coll").value.trim(), sensualite: $("#s-sens").value.trim()}});
    toast("Slogan et phrases enregistrés");
  });
  $("#k-save").addEventListener("click", async ()=>{
    await act("POST", "/admin/config", {config: {
  ...
  $("#px-save").addEventListener("click", async ()=>{
    await act("POST", "/admin/config", {config: {facebookPixelId: $("#px-id").value.replace(/\s/g,"")}});
    toast($("#px-id").value.trim() ? "Pixel Facebook activé ✓" : "Pixel Facebook désactivé");
  });
  $("#bk-export").addEventListener("click", async ()=>{
    const r = await act("GET", "/admin/backup");
    const a = document.createElement("a");
  ...
  $("#bk-import").addEventListener("change", e=>{
    ...
    r.onload = async ()=>{
      try{
        ...
        await act("POST", "/admin/restore", {backup: data});
        toast("Sauvegarde restaurée"); await show("home");
  ...
  $("#w-save").addEventListener("click", async ()=>{
    try{
      await act("POST", "/admin/password", {old: $("#w-old").value, new: $("#w-new").value});
...
async function init(){
  $("#login-form").addEventListener("submit", async e=>{
    e.preventDefault();
    try{
      const r = await API.post("/admin/login", {password: $("#adm-pwd").value});
      API.setAdminToken(r.token);
      $("#adm-pwd").value = "";
      $("#admin-login").hidden = true;
      $("#admin-shell").hidden = false;
      show("home");
    }catch(err){ $("#adm-pwd").value = ""; $("#adm-pwd").placeholder = err.message; }
  });
  $("#adm-logout").addEventListener("click", ()=>{ API.setAdminToken(null); location.reload(); });
  if(API.adminToken()){
    try{
      S = await API.get("/admin/state", true);
      $("#admin-login").hidden = true;
      $("#admin-shell").hidden = false;
      show("home");
      return;
    }catch(e){ API.setAdminToken(null); }
  }
  showLogin();
}
